function forceLayout(){
        ...
        .on("mouseover", function(d){
            d3.select(this)
                ...
                .style("stroke-width", function(d){
                    return nominal_stroke * (d.size + 1)
                });
    ...
    var circle = node.append("path")
        ...
        .size(function(d) { return Math.PI*Math.pow(size(d.size),2); })
    ...
    function showForceTooltip(c,node){
        var index = node.id;
        ...
        force_tooltip.transition().duration(200).style("opacity", .9);

        var x = matrix.e + xOffset,
            y = matrix.f + yOffset;

        force_tooltip.html("</p><p class='center-align'>Name: " + index)
            .style("left", x + "px")     
            .style("top", y + "px");
    };

    function hideForceTooltip(){
        force_tooltip.transition().duration(200).style("opacity", 0);
    };

    function showLinkTooltip(d, c){
        ...
        for(var i = 0; i<d.devices.length; i++){
            //entry = (i+1).toString() + ". " + d.devices[i]
            var entry = d.devices[i];
            if(i+1 == d.devices.length){
                entry += "<br>";
            }else{
                entry += "<br><br>";
            }
        ...
        link_tooltip.transition().duration(200).style("opacity", .9)
        link_tooltip.html(text)
            .style("left", x + "px")     
            .style("top", y + "px");
    };
